...
wave.sine <- function(f=440, numSamples=22050, sampleRate=44100, pm=rep(0, numSamples), fm=rep(0, numSamples), am=rep(1, numSamples)) {
  ...
  wave <- sin(2 * pi * (freqs * 1:numSamples + phase) / sampleRate) * am # Amplitude modulation (unit = original amplitude)
  wave / max(abs(wave))
}

wave.series <- function(f=440, numSamples=22050, sampleRate=44100, detune=1) { # detune 1 = saw, 2 = square, etc...
  wave <- numeric(numSamples)
  harmonic.numbers <- seq(1, (sampleRate / 2) %/% 440, detune)
  wave <- rowSums(vapply(harmonic.numbers, function(harmonic.number) { wave.sine(f=f*harmonic.number, numSamples=numSamples, sampleRate=sampleRate) / harmonic.number }, rep(0, harmonic.numbers)))
  wave / max(abs(wave))
}
